...
class CIFARResNetV2(HybridBlock):
    ...
    def __init__(self, block, layers, channels, classes=10,
        ...
        with self.name_scope():
            ...
            self.weight = self.params.get('dense0_weight',
                                          shape=(classes,channels[-1]),
                                          allow_deferred_init=True)
            
            self.bias = self.params.get('dense0_bias',
                                          shape=(classes,),
                                          allow_deferred_init=True)
...
#            self.output = nn.Dense(classes, in_units=in_channels)
    ...
    def hybrid_forward(self, F, x, weight, bias):
        x = self.features(x)
        x1 = F.FullyConnected(x,weight=weight,bias=bias,num_hidden=self.classes)
#        x1 = self.output(x)
        if self.norm:
            ...
            x2 = F.FullyConnected(x2,weight=weight,bias=bias,num_hidden=self.classes)
        
#        weight1 = F.L2Normalization(weight)
...
def cifar_resnet20_v2(**kwargs):
    ...
    channels = [16, 16, 32, 64]
    layers = [n] * (len(channels) - 1)    
    net = CIFARResNetV2(CIFARBasicBlockV2, layers, channels, **kwargs)
...
def cifar_resnet110_v2(**kwargs):
    ...
    norm_kwargs : dict
        ...
        for :class:`mxnet.gluon.contrib.nn.SyncBatchNorm`.
    """
    return get_cifar_resnet(2, 110, pretrained=False, **kwargs)
